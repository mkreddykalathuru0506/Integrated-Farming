...
- **Phase 9 (hardening & prod):** ✅ code complete (9.0–9.3 merged); **production deploy + live VPS cutover remain the owner §3 checkpoint** (not done). 9.0 debt: CI actions `checkout/setup-node@v5` (Node-24), seller **GSTIN** now printed on invoice PDF (pure `invoiceHeaderLines`). 9.1: `express-rate-limit` on `/api/auth/*` (429), `/api/health/ready` readiness (DB), `docs/security-review.md` (controls + findings: audit-log wiring, CORS allowlist — both pre-prod must-do). 9.2: `scripts/{backup,restore}.sh` (pg_dump/retention) + `scripts/load-sanity.mjs` (no-dep) + npm `db:backup`/`db:restore`/`load:sanity`; api+web container healthchecks; `docs/monitoring.md`. 9.3: `migrations.down.test` guards every migration has up+down.sql; **rollback rehearsed** (transactional down.sql → ROLLBACK); `docs/runbook.md` full Production section. **224 api tests, 17 web tests.** Verified locally: readiness db:true, load 3085 rps/p99 20ms, pg_dump 141KB. New deps: `exceljs` (8.1), `express-rate-limit` (9.1) — both MIT.
