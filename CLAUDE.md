...
- **Slice 0.5 (app shell + design system + i18n + PWA):** built & verified (33 tests; PWA build emits sw.js + workbox + manifest with 192/512+maskable icons; gates green) — on branch `phase-0/slice-0.5-shell-pwa`, **awaiting end-of-slice checkpoint + merge**.
- **Web stack additions:** design-system primitives in `apps/web/src/ui` (cn via clsx+tailwind-merge; Button/Input/Select/Card/Badge via cva); `AppShell` (sticky header, 360px); `LanguageToggle` (en + hi seed, fallback en); PWA via `vite-plugin-pwa` (autoUpdate, offline app-shell precache); placeholder icons generated by `apps/web/scripts/gen-icons.mjs` (replace with real art later).
- **Deferred:** install/Lighthouse/visual a11y pass → 0.8; offline-write + Playwright e2e → Phase 2.
- **Next:** 0.6 staging deploy (Hostinger VPS) → completes Phase 0.
