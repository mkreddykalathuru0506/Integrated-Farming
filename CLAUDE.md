...
- **Phase 10 (UI redesign — owner-requested):** 🔄 Milestone 1 in progress on `phase-10/ui-redesign-m1`. Full visual redesign (backend untouched — purely additive frontend). New design system "Earth green + harvest gold" (semantic HSL tokens in `index.css` + `tailwind.config.ts`; Plus Jakarta Sans + Inter + Noto Sans Devanagari). Adopted shadcn-style primitives (Radix `DropdownMenu` + `Sheet`) alongside refit cva primitives. New app frame: deep-green collapsible **sidebar** (9 grouped sections, `components/nav.tsx` maps all 27 panels), **topbar** (farm switcher + language toggle + user menu), responsive **mobile drawer** at <lg. New **Dashboard** landing (`farm/Dashboard.tsx`): 6 KPI stat cards (P&L, open risks, active batches, low-stock feed, temp, alerts) + Recharts (risk-severity bar, market-rates bar) + open-risk/alert lists — all from real endpoints. Replaced the old one-column AppShell (deleted `AppShell.tsx`/`FarmsPanel.tsx`); i18n `nav` namespace added (en+hi, parity-enforced; **14** parity tests). New deps (MIT/ISC): `recharts`, `lucide-react`, `@radix-ui/react-{dropdown-menu,dialog,tooltip}`, `tailwindcss-animate`. Verified: web typecheck/lint/build green, **18 web tests**, Playwright screenshots confirm desktop+mobile render with no page errors. **Next (M2):** restyle individual panels (tables/forms), per-section multi-column layouts, route-based deep-linking. Not yet merged.
