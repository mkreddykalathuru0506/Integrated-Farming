...
| E2E tests (Playwright, self-contained: boots api :4100 + web :5190) | `pnpm --filter @ifm/web e2e` |
| Domain-logic coverage gate (≥80% lines, ~4s) | `pnpm --filter @ifm/api test:coverage` |
...
**Gotcha (git worktrees):** a fresh `pnpm install` in a new worktree needs `pnpm db:generate` re-run afterwards, or typecheck explodes with phantom Prisma type errors (the generated client lands in the worktree's own `node_modules`).
...
- **Phase 11 (app completion — owner-requested):** in progress. **Merged:** 11.1 foundation (data layer, **session persistence** via stored+rotated refresh token w/ single-flight refresh, toast/dialog/table **UI kit**); 11.2 shell UX (**per-panel routes** `/section/panel`, **command palette** Ctrl/Cmd+K, **notification bell**, mobile tab bar); 11.4a global **search + activity feed** APIs; 11.5a read-side completeness (**pagination envelope** `?page[&size]` + missing read endpoints); 11.6a–e **27-panel sweep** (every feature panel rebuilt on the query/table/dialog kit); 11.7 **dashboard + proactive intelligence** (THI heat-stress rules, mortality-spike detection, 05:30 IST sweep, onboarding, rebuilt dashboard); 11.8b QA/test hardening (below). **Four OPEN owner-checkpoint PRs (§1.4 — do NOT auto-merge):** **#55** auth API + migration (OTP login, password reset, profile + sessions); **#58** membership lifecycle (add / change-role / deactivate farm members); **#60** entity lifecycles (expense soft-delete **migration**, invoice mark-paid/void, entity PATCH); **#62** auth web UI (register/OTP/reset/sessions — depends on #55). **Tests: 308 api, 265 web, 3 shared, 5 Playwright e2e journeys.** **Known debt:** server pagination only partially adopted by panels (most lists still client-side); `farm/DashboardPanel.tsx` deletion still owner-gated (§3); BullMQ engines untested in CI; PWA manifest strings English-only (manifest can't be localised at runtime).
  - **Slice 11.8b (QA hardening):** ✅ — the Brief §11 e2e journeys now exist in `apps/web/e2e/`: **add-batch**, **invoice** (2 lines, different GST rates → FY number + CGST/SGST split in detail), **dispatch-coldchain** (API-seeded confirmed order + FROZEN lot; non-refrigerated dispatch blocked with explanation → compliant retry dispatches), **withdrawal-block** (medicated batch blocked from processing with explanation), plus offline-sync (updated for per-panel routes → `/daily/logs`). `playwright.config.ts` **webServer array** starts api (:4100, `NODE_ENV=test` → no BullMQ, no auth rate-limit cap) + web (:5190, `VITE_API_URL`) so `pnpm --filter @ifm/web e2e` is self-contained (prereq: migrated+seeded DB; `reuseExistingServer` locally, never in CI). `e2e.yml` runs on every PR as a **separate non-required workflow** (informational — flaky browser runs can't block merges). **Coverage gate:** `@ifm/api test:coverage` (vitest v8, 80% lines threshold, currently **100%**) over the 12 pure domain-logic modules (stage-machine, counts, withdrawal, gst, feed/finance/sales/cold/dispatch calc, circularity, rules, schedule.calc), wired into CI (~4s — instrumenting the full DB-backed suite deterministically crashes a tinypool fork at teardown, so the script pins the 13 pure unit-test files). Flake hardening: `vi.setConfig({ testTimeout: 20_000 })` applied to 17 dialog-heavy web test files. New dev dep: `@vitest/coverage-v8` (MIT).
- **Phase 10 (UI redesign — owner-requested):** ✅ M1+M2+M3 complete (backend untouched — purely additive frontend). **M1 (shell):** new app frame — collapsible **sidebar** (9 grouped sections, `components/nav.tsx` maps all 27 panels), **topbar** (farm switcher + language toggle + user menu), responsive **mobile drawer** at <lg; replaced the old one-column AppShell (deleted `AppShell.tsx`/`FarmsPanel.tsx`); adopted Radix `DropdownMenu` + `Sheet` primitives. **M2 ("Harvest" theme):** warm editorial design system — bone/paper canvas + deep-pine gradient sidebar + harvest-gold accents (semantic HSL tokens in `index.css` + `tailwind.config.ts`), **Fraunces** (display/numbers) + **Hanken Grotesk** (UI) + **JetBrains Mono** (data) + Noto Sans Devanagari fallback. Rebuilt **Dashboard** (`farm/Dashboard.tsx`): time-of-day greeting, profit hero (compact ₹ L/Cr) + KPI row, **risk-severity donut** (Recharts), **cost-by-category** gradient bars, **cold-chain SVG gauge**, market bars, activity timeline, actionable open-risk flags, contour texture — all real data, graceful empty states. i18n `nav` + dashboard keys (en+hi, parity-enforced; **14** parity tests). New deps (MIT/ISC): `recharts`, `lucide-react`, `@radix-ui/react-{dropdown-menu,dialog,tooltip}`, `tailwindcss-animate`. Verified: web typecheck/lint/build green, **18 web tests**, Playwright screenshots (desktop+mobile, no page errors). **M3 (panel restyle):** the 27 inner feature panels no longer use hardcoded cold `slate/red/amber` greys — all now render on the Harvest **semantic tokens** via a small shared panel kit `ui/Panel.tsx` (`PanelHeading`, `PanelNote`, `PanelError`, `SubPanel`, `DataRow`). ~300 hardcoded palette classes removed; raw `<select>`s → `Select` primitive, ad-hoc status pills → `Badge` variants, numeric spans get `.tabular`. className-only — no logic/JSX-structure/i18n changes (18 web tests + parity unchanged). Verified: web typecheck/lint/build green, **0** stale palette classes across live panels (grep) and **0** in the live DOM; computed styles confirmed resolving to Harvest HSL (heading `muted-foreground` rgb(81,103,90), sub-surface `secondary/60` rgb(241,235,218), paper canvas) via an in-page check (dev-server/extension network-idle handshake blocked a pixel screenshot — a tooling quirk; `readyState` complete, no console errors). Demo farm seeded via API with sample expenses/market/cold-storage for a populated dashboard. **Known debt:** `farm/DashboardPanel.tsx` is now **dead code** (superseded by `Dashboard.tsx`, zero refs) — left in place pending owner OK to delete (file deletion = §3). **Dark mode:** ✅ done — class-based (`darkMode:'class'` + a `.dark` token block in `index.css`; components need **zero** `dark:` classes since they're all token-driven). Warm "dusk" palette (deep-pine near-black canvas, off-white ink, brightened pine/gold/leaf accents; sidebar stays the dark-green anchor). `components/ThemeToggle.tsx` in the topbar (Sun/Moon, i18n `nav.themeDark`/`themeLight` en+hi) toggles + persists to `localStorage['ifm.theme']` and follows OS `prefers-color-scheme` until the user chooses; a no-flash `<script>` in `index.html` applies the theme before first paint. Also fixed `LanguageToggle` (was still on hardcoded `slate`, missed by the farm/-scoped M3 sweep). Verified live: every semantic token flips to the dark palette (computed styles — canvas rgb(14,22,19), ink rgb(236,232,223), gold rgb(232,174,48)), 0 stale palette classes, all static gates green. **Route-based deep-linking:** ✅ done — each sidebar section has a real URL (`/`=overview, `/livestock`, `/finance`, …) via a tiny dependency-free History-API router (`components/router.ts` `useRoute()` + `pathForSection`/`sectionFromPath`; no react-router — the nav is a flat section list). `AppLayout` derives `activeKey` from the URL and canonicalises unknown paths to their fallback (→`/`); sidebar items are now real `<a href>` (middle/ctrl-click open new tabs) with plain-click SPA nav. Works in dev (Vite history fallback) + prod (nginx `try_files … /index.html`, already present) — no infra change. `router.test.ts` (3 tests, **21 web tests** total). Verified live in-browser: deep-link/unknown→`/` normalization, sidebar click updates URL+active+title, real Back button (`/livestock`→`/finance`), no console errors. **Deferred (not done):** per-panel pixel screenshots against seeded data remain a staging check (local Docker was down); farm id is not in the URL (farm stays in the header switcher).
